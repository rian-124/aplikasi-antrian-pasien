...
useEffect(() => {
  ...
  const fetchPatients = async () => {
    ...
    try {
      ...
      const [patientRes, loketRes] = await Promise.all([
        ...
        fetch("http://192.168.50.222:4000/api/lokets", {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const handleCallPatient = async (patient: Patient) => {
    ...
    try {
      const res = await fetch(
        `http://192.168.50.222:4000/api/antrian-pasien/${patient.id}`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleUpdateUserLoket = async (loketId: string) => {
    ...
    try {
      const res = await fetch("http://192.168.50.222:4000/api/users/loket", {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-type": "application/json",
  ...
  const handleCheckoutUserLoket = async () => {
    ...
    try {
      const res = await fetch(
        "http://192.168.50.222:4000/api/users/loket/checkout",
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const handleRecallPatient = async (patient: Patient) => {
    ...
    if (recallCount >= 2) {
      await fetch(`http://192.168.50.222:4000/api/antrian-pasien/${patient.id}`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
