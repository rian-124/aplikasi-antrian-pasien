    ...
    const fetchPatients = async () => {
      try {
        ...
        const [patientRes, outletRes] = await Promise.all([
          fetch("http://192.168.1.19:4000/api/antrian-pasien", { headers: { 'Authorization': `Bearer ${token}` } }),
          fetch("http://192.168.1.19:4000/api/outlet", { headers: { 'Authorization': `Bearer ${token}` } })
        ]);
