...
export class UserService {
  static async fetchUsers(): Promise<User[]> {
    ...
    const res = await fetch("http://192.168.50.2:4000/api/users", {
      headers: {
  ...
  static async deleteUser(userId: number) {
    const res = await fetch(`http://192.168.50.2:4000/api/users/${userId}`, {
      method: 'DELETE',
      headers: {
  ...
  static async fetchRoles() {
    ...
    const res = await fetch('http://192.168.50.2:4000/api/roles', {
      headers: {
        'Authorization': `Bearer ${token}`,
      },
    });
  ...
  static async fetchOutlets() {
    ...
    const res = await fetch('http://192.168.50.2:4000/api/outlet', {
      headers: {
        'Authorization': `Bearer ${token}`,
      },
    });
  ...
  static async updateUser(updatedUser: User) {
    ...
    const res = await fetch(`http://192.168.50.2:4000/api/users/${updatedUser.id}`, {
      method: 'PATCH',
      headers: {
    ...
    if (!res.ok) {
      ...
      try {
        ...
        throw new Error(errorJson.message || `Failed to update user: HTTP ${res.status}`);
      } catch (e) {
        throw new Error(errorText || `Failed to update user: HTTP ${res.status}`);
      }
    }
  ...
  static async createUser(userData: {
    ...
    if (!res.ok) {
      ...
      try {
        const errorJson = errorText ? JSON.parse(errorText) : {};
        throw new Error(errorJson.message || `Failed to create user: HTTP ${res.status}`);
      } catch (e) {
        throw new Error(errorText || `Failed to create user: HTTP ${res.status}`);
      }
    }
