...
export class UserService {
  ...
  static async deleteUser(userId: number) {
    const res = await fetch(`http://172.20.10.2:4000/api/users/${userId}`, {
      method: 'DELETE',
      headers: {
  ...
  static async fetchRoles() {
    ...
    const res = await fetch('http://172.20.10.2:4000/api/roles', {
      headers: {
        'Authorization': `Bearer ${token}`,
      },
    });
  ...
  static async fetchOutlets() {
    ...
    const res = await fetch('http://172.20.10.2:4000/api/outlet', {
      headers: {
        'Authorization': `Bearer ${token}`,
      },
    });
  ...
  static async updateUser(updatedUser: User) {
    ...
    const res = await fetch(`http://172.20.10.2:4000/api/users/${updatedUser.id}`, {
      method: 'PATCH',
      headers: {
