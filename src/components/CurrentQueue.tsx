...
export default function CurrentQueue({ patient }: { patient: Patient }) {
  return (
    <>
      ...
      <div className="border-b border-gray-700 w-full text-center">
        <h2 className="text-black mb-2 text-3xl mt-4 font-semibold">Loket</h2>
        <p className="text-4xl font-extrabold mb-4 text-green-400">{patient.loket}</p>
      </div>
      <div className="w-full text-center">
        <h2 className="text-black mb-2 text-3xl font-semibold mt-4">Status</h2>
        <p className="text-4xl font-bold text-blue-400">{patient.status}</p>
      </div>
    </>
  );
}
